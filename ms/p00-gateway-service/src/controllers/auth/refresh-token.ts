...
export class Refresh {
  static async token(req: Request, res: Response): Promise<void> {
    try {
      ...
      console.error(error.response, new BadGatewayError(error.message, 'GatewayService Refresh.token method error'));
      res.status(StatusCodes.BAD_GATEWAY).json({
        message: 'Failed to refresh token from authorization service.',
        error: new BadGatewayError(error.message, 'GatewayService Refresh.token method error')
      });
    }
  }
}
