...
export async function create(req: Request, res: Response, _next: NextFunction): Promise<void> {
  ...
  if (result?.id) {
    const uploadResult: UploadApiResponse = (await uploads(profilePicture, `${result.profilePublicId!}`, true, true)) as UploadApiResponse;
    ...
    res.status(StatusCodes.NOT_FOUND).json({ message: 'User creation failed.' });
  }
  // } catch (err) {
  //   next(err);
  // }
